...
module Position = struct
  ...
  let compare p1 p2 =
    ...
    | NoPosition, _ -> -1
    | _, NoPosition -> 1
    | Position (l1, c1), Position (l2, c2) -> [%compare: int * int] (l1, c1) (l2, c2)
end
...
module Span = struct
  ...
  let ( <+> ) (s1, e1) (s2, e2) =
    Comparable.min Position.compare s1 s2, Comparable.max Position.compare e1 e2
end
...
module Location = struct
  ...
  let ( <+> ) { file_path = fp1; span = s1 } { file_path = fp2; span = s2 } =
    if String.equal fp1 fp2
    then { file_path = fp1; span = Span.(s1 <+> s2) }
...
let fold ls = Location.(ls |> List.map ~f:location |> List.fold_left ~f:( <+> ) ~init:empty)
